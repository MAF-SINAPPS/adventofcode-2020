...
";
        ...
        public static void Part1() =>
            Console.WriteLine(
                "Day 2 part 1: " +
                _input.Split(Environment.NewLine, StringSplitOptions.RemoveEmptyEntries).Count(_ =>
                 {
                     var split = Regex.Match(_, @"(?<min>\d+)-(?<max>\d+) (?<char>[a-z]): (?<password>\w+)", RegexOptions.IgnoreCase);
                     return Enumerable.Range(Convert.ToInt32(split.Groups["min"].Value), Convert.ToInt32(split.Groups["max"].Value) - Convert.ToInt32(split.Groups["min"].Value) + 1)
                        .Contains(split.Groups["password"].Value.Count(__ => __ == Convert.ToChar(split.Groups["char"].Value)));
                 }));
    }
}
